    ...
    while (my $line = <$in>) {
        ...
        if ($line =~ m/$pattern/) {
            ...
            print STDERR 'matches /st/: ', $file, "\n", '$matched_count: [',
                $matched_count, '/', $file_count, '] $line: ', $line, "\n"
...
sub check_line {
    ...
    for my $ocr_line (@ocr_lines) {
        ...
        if (similarity($ocr_line, $line) > $best_similarity && $ocr_line =~ m/ſ/) {
            ...
            print STDERR '$line:     ', $line,     "\n" if ($options->{'verbose'} >= 1);
            print STDERR '$ocr_line: ', $ocr_line, "\n" if ($options->{'verbose'} >= 1);
        } ## end if (similarity($ocr_line...))
    } ## end for my $ocr_line (@ocr_lines)
...
sub read_ocr {
    ...
    if (-e $ocr_txt_file && -f $ocr_txt_file) {
        ...
        open(my $in, "<:encoding(UTF-8)", $ocr_txt_file)
            or die "cannot open $ocr_txt_file: $!";
